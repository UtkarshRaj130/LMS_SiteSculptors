...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    try {
      ...
      const bookIndex = user.reservedBooks.findIndex(
        (reservedBook) => reservedBook._id.toString() === theBook._id
      );

      if (bookIndex === -1) {
        return res.status(400).json({ message: 'Book not found in reserved books' });
      }
      ...
      const now = new Date();
      const returningTime = now.toTimeString().split(' ')[0]; // Get HH:MM:SS
      const returningDate = formatDate(now); // Format date as YYYY-MM-DD

      const returnedBook = {
        ...user.reservedBooks[bookIndex].toObject(),
        returningTime,
        returningDate,
      };

      user.returnedBooks.push(returnedBook);
      user.reservedBooks.splice(bookIndex, 1);
      book.count += 1;
